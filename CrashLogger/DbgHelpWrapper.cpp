...
private:
	...
	DbgHelpWrapper() {
		...
		if (!CopyFileW(dbghelp_path, temp_dbghelp_path, FALSE)) {
			//_MESSAGE("%s", std::format("Cannot create a copy of DbgHelp.dll, error: {} ({:08X})", GetErrorAsString(GetLastError()), GetLastError()));
		...
		if (!dbghelp_dll) {
			...
			//_MESSAGE("%s", std::format("Cannot load DbgHelp.dll, error: {} ({:08X})", GetErrorAsString(GetLastError()), GetLastError()));
			return;
		}
